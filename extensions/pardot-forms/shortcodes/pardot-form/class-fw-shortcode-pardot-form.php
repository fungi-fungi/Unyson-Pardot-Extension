...
class FW_Shortcode_Pardot_Form extends FW_Shortcode
{
	private $restricted_types = array( 'pardot-form' );
	...
	public function _init()
	{
		...
		add_filter( 'fw_ext:shortcodes:collect_shortcodes_data', array(
			$this, '_filter_add_pardot_form_data'
		) );
	}
	 ...
	 * @internal
	 */
	public function _filter_add_pardot_form_data( $structure ) {
	...
	public function _action_register_builder_item_types() {
		if (fw_ext('page-builder')) {
			require $this->get_declared_path('/includes/item/class-page-builder-pardot-form-item.php');
		}
	}
	...
	protected function _render($atts, $content = null, $tag = '')
	{
		$form_data = array(
			...
			'pardot_link'     => $atts['pardot_link'],
		);

		/**
		 * @var FW_Extension_Contact_Forms $extension
		 */
		$extension = fw_ext('pardot-forms');
	...
	public function get_item_data() {
		 ...
		 */
		$shortcode = fw_ext( 'shortcodes' )->get_shortcode( 'pardot_form' );
